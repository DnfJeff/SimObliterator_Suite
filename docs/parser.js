...
import { TextReader, BinaryReader, BinaryWriter, decompressFloats, compressFloats, } from './reader.js';
...
function parseAnimData(r) {
    const result = { skeletons: [], suits: [], skills: [] };
...
export function parseBMF(buffer) {
    const r = new BinaryReader(buffer);
    const name = r.readString();
    const textureName = r.readString();
...
export function parseCFP(buffer, skill) {
    ...
    if (skill.numRotations > 0) {
        ...
        for (let i = 0; i < skill.numRotations; i++) {
            skill.rotations.push(quat(floats[i * 4], floats[i * 4 + 1], floats[i * 4 + 2], -floats[i * 4 + 3]));
        }
    }
}
// ============================================================
...
function writeTimeProps(w, timeProps) {
...
function writeMotion(w, motion) {
    ...
    w.writeBool(hasProps);
    if (hasProps)
        writeProps(w, motion.props);
    const hasTimeProps = motion.timeProps.size > 0;
    w.writeBool(hasTimeProps);
    if (hasTimeProps)
        writeTimeProps(w, motion.timeProps);
}
function writeSkill(w, skill) {
...
export function writeReport(cmx, meshes = []) {
    const L = [];
...
export function writeCFP(skill) {
    const w = new BinaryWriter();
