...
export function findRoot(bones) {
    return bones.find(b => !b.parent) ?? null;
}
// Find a bone by name
export function findBone(bones, name) {
...
export function updateTransforms(bones) {
    const root = findRoot(bones);
    if (!root)
        return;
    propagate(root, vec3(), quat());
}
function propagate(bone, parentPos, parentRot) {
...
const _deformLogged = new Set();
export function deformMesh(mesh, bones, boneMap) {
    const totalVerts = mesh.vertices.length;
    const boundCount = mesh.uvs.length; // bound vertices = UV count
    const outVerts = new Array(totalVerts);
    const outNorms = new Array(totalVerts);
    ...
    for (const binding of mesh.boneBindings) {
        ...
        for (let i = 0; i < binding.vertexCount; i++) {
            ...
            if (vi < totalVerts && mesh.vertices[vi] && mesh.normals[vi]) {
                outVerts[vi] = vec3Add(bone.worldPosition, quatRotateVec3(bone.worldRotation, mesh.vertices[vi]));
    ...
    for (let i = 0; i < mesh.blendBindings.length; i++) {
        ...
        if (blendVi < totalVerts && destVi < totalVerts &&
            ...
            if (w >= 1.0) {
                ...
                outVerts[destVi] = vec3(ow * dv.x + w * bv.x, ow * dv.y + w * bv.y, ow * dv.z + w * bv.z);
                outNorms[destVi] = vec3Normalize(vec3(ow * dn.x + w * bn.x, ow * dn.y + w * bn.y, ow * dn.z + w * bn.z));
            }
            blendCount++;
        }
    }
