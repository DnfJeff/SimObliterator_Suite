...
export function parseCMX(text: string): CMXFile {
    ...
    console.log(`[parseCMX] v${version} skeletons=${result.skeletons.length} suits=${result.suits.length} skills=${result.skills.length}`,
        { skeletons: result.skeletons.map(s => ({ name: s.name, bones: s.bones.length })),
...
export function parseBCF(buffer: ArrayBuffer): CMXFile {
    ...
    console.log(`[parseBCF] skeletons=${result.skeletons.length} suits=${result.suits.length} skills=${result.skills.length}`,
        { skeletons: result.skeletons.map(s => ({ name: s.name, bones: s.bones.length })),
          suits: result.suits.map(s => ({ name: s.name, skins: s.skins.map(sk => sk.meshName) })),
          skills: result.skills.map(s => s.name) });
